  ...
  return (
    <section
      className={getSectionStyles(
        settings || {backgroundColor: backgroundVariant === 'dark' ? 'navy' : 'white'},
      )}
    >
      <div className="container mx-auto px-4 lg:px-8">
        ...
        >
          {displayStats.map((stat) => (
            <div key={stat._key} className="text-center group">
              <div className="text-4xl md:text-6xl font-heading font-bold text-faect-blue mb-4 group-hover:scale-110 transition-transform">
                {stat.value}
              </div>
              <div className="text-lg font-bold mb-2">{stat.label}</div>
              {stat.description && <p className="text-faect-gray text-sm">{stat.description}</p>}
            </div>
          ))}
        </div>
      </div>
